...
export function fulfillOrder(progress:Progress,random=Math.random){if(progress.milk<GAME_CONFIG.orderMilk||progress.fish<GAME_CONFIG.orderFish||progress.cheese<GAME_CONFIG.orderCheese)return null;return {...progress,fish:progress.fish-GAME_CONFIG.orderFish+GAME_CONFIG.orderFishReward,milk:progress.milk-GAME_CONFIG.orderMilk,cheese:progress.cheese-GAME_CONFIG.orderCheese,medals:progress.medals+GAME_CONFIG.orderMedalReward,diamonds:progress.diamonds+(random()<GAME_CONFIG.orderDiamondChance?1:0),playerXp:progress.playerXp+GAME_CONFIG.orderXpReward,orderIndex:progress.orderIndex+1}}
