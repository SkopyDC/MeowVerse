...
try{
  for(const viewport of viewports){
    ...
    await page.screenshot({path:`artifacts/rebuild-v3/${viewport.name}/01-island.png`});
    ...
    await page.screenshot({path:`artifacts/rebuild-v3/${viewport.name}/02-pond-sheet.png`});
    ...
    await page.screenshot({path:`artifacts/rebuild-v3/${viewport.name}/03-collected.png`});
  ...
  const page=await context.newPage();await page.goto("http://127.0.0.1:4173",{waitUntil:"domcontentloaded"});await page.waitForSelector("canvas");assert(await page.locator(".v3-fatal").count()===0,"clean start failed");await context.close();console.log("✓ čistý start");
}finally{
